...
def extract_brand_from_tags(tags):
    ...
    if "brand" in tags:
        return tags["brand"]
    # 最后 fallback：尝试兼容品牌 compatible_brands 的首个词
    if "compatible_brands" in tags:
        return tags["compatible_brands"].split()[0]
    return ""

def get_main_format(fmt_name, brand, ext, encoder=""):
    ext = ext.lower().lstrip('.')  # 去掉扩展名前缀点
    ...
    # 如果 mainfmt 不在 fmt_name 的逗号分隔列表里，才用 fmt_name 覆盖
    fmt_name_set = set(fmt_name.split(','))
    if mainfmt not in fmt_name_set:
        mainfmt = fmt_name
    #"mov,mp4,m4a,3gp,3g2,mj2", "matroska,webm" 这两个特殊情况后续处理

    # 3. brand 覆盖判断，补充更多常见brand关键字
    if "qt" in brand:
        mainfmt = "mov"
    elif "mp4" in brand:
        mainfmt = "mp4"
    elif any(x in brand for x in ["webm", "vp8", "vp9", "vp09", "vpx "]):
        mainfmt = "webm"
    else:
        if "libwebm" in encoder or "webm" in encoder:
            mainfmt = "webm"
        elif "libmatroska" in encoder or "libebml" in encoder:
            mainfmt = "mkv"
...
def main(file1, file2):
    ...
    tags1 = fmt1.get("tags", {})
    tags2 = fmt2.get("tags", {})
    brand1 = extract_brand_from_tags(tags1)
    brand2 = extract_brand_from_tags(tags2)    
    encoder1 = tags1.get("encoder", "")
    encoder2 = tags2.get("encoder", "")
    mainfmt1 = get_main_format(fmt1.get("format_name"), brand1, ext1, encoder1)
    mainfmt2 = get_main_format(fmt2.get("format_name"), brand2, ext2, encoder2)
